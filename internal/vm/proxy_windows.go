...
// (thrive ps, thrive run, etc.) connect to when the VM is running. Windows
// 10+ supports AF_UNIX natively, so this mirrors proxy_darwin.go exactly.
...
func ServeControlSocket(ctx context.Context, cfg *Config, bridge Bridge) {
	...
	// mu serialises commands to thrived — the vsock/hvsock bridge is single-stream.
	var mu sync.Mutex
...
// syncImageToVM transfers a locally pulled image to the VM via the bridge.
